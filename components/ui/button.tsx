...
import { useMagneticEffect } from "@/lib/use-magnetic";
...
export interface ButtonProps
  ...
  asChild?: boolean;
  /**
   * Subtle magnetic hover — translates toward cursor on desktop. No-op on
   * touch / coarse pointer / prefers-reduced-motion. Opt-in to keep regular
   * buttons free of the listener.
   */
  magnetic?: boolean;
}
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, magnetic = false, ...props }, ref) => {
    ...
    // Bridge our internal ref to the forwarded one (so consumer .focus() still works).
    React.useImperativeHandle(ref, () => innerRef.current as HTMLButtonElement, []);
    useMagneticEffect(innerRef, { enabled: magnetic, pull: 3 });
    return <Comp ref={innerRef} className={cn(buttonVariants({ variant, size, className }))} {...props} />;
  },
);
